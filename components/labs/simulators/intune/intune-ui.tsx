  ...
  return (
    <div className={styles.bladeFrame}>
      <div className={styles.bladeNav}>
        {sections.map((s) => (
          <div key={s} className={`${styles.bladeNavItem} ${activeSection === s ? styles.bladeNavItemActive : ""}`} onClick={() => onSectionChange(s)}>
            {s}
          </div>
        ))}
      </div>
      <div className={styles.bladeBody}>
        <div className={styles.bladeHeader}>
          <div className={styles.bladeTitle}>{title}</div>
        </div>
        {toolbar ? <div className={styles.bladeToolbar}>{toolbar}</div> : null}
        {children}
      </div>
    </div>
  );
...
export function BladeActionButton({ label, onClick, danger }: { label: string; onClick: () => void; danger?: boolean }) {
  return (
    <button type="button" className={`${styles.bladeActionBtn} ${danger ? styles.bladeActionBtnDanger : ""}`} onClick={onClick}>
      {label}
    </button>
  );
}

export function SubRailLayout({ rail, children }: { rail: ReactNode; children: ReactNode }) {
  return (
    <div className={styles.withSub}>
      <div className={styles.subrail}>{rail}</div>
      <div className={styles.subrailContent}>{children}</div>
    </div>
  );
}

export function SubRailItem({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <div className={`${styles.subrailItem} ${active ? styles.subrailItemActive : ""}`} onClick={onClick}>
      {label}
    </div>
  );
}

export function SubRailSection({ children }: { children: ReactNode }) {
  return <div className={styles.subrailSection}>{children}</div>;
}
